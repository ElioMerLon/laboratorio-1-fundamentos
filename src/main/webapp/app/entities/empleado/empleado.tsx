  ...
  return (
    <div>
      <h2 id="empleado-heading" data-cy="EmpleadoHeading">
        <Translate contentKey="fsijhipsterApp.empleado.home.title">Empleados</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="fsijhipsterApp.empleado.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/empleado/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="fsijhipsterApp.empleado.home.createLabel">Create new Empleado</Translate>
          </Link>
        </div>
      </h2>
          ...
          {empleadoList && empleadoList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="fsijhipsterApp.empleado.id">ID</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
                  <th className="hand" onClick={sort('nombres')}>
                    <Translate contentKey="fsijhipsterApp.empleado.nombres">Nombres</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('nombres')} />
                  </th>
                  <th className="hand" onClick={sort('apellidos')}>
                    <Translate contentKey="fsijhipsterApp.empleado.apellidos">Apellidos</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('apellidos')} />
                  </th>
                  <th className="hand" onClick={sort('correo')}>
                    <Translate contentKey="fsijhipsterApp.empleado.correo">Correo</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('correo')} />
                  </th>
                  <th className="hand" onClick={sort('nroCelular')}>
                    <Translate contentKey="fsijhipsterApp.empleado.nroCelular">Nro Celular</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('nroCelular')} />
                  </th>
                  <th className="hand" onClick={sort('fechacontrato')}>
                    <Translate contentKey="fsijhipsterApp.empleado.fechacontrato">Fechacontrato</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('fechacontrato')} />
                  </th>
                  <th className="hand" onClick={sort('salario')}>
                    <Translate contentKey="fsijhipsterApp.empleado.salario">Salario</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('salario')} />
                  </th>
                  <th className="hand" onClick={sort('comisionPorcentaje')}>
                    <Translate contentKey="fsijhipsterApp.empleado.comisionPorcentaje">Comision Porcentaje</Translate>{' '}
                    <FontAwesomeIcon icon={getSortIconByFieldName('comisionPorcentaje')} />
                  </th>
                  <th>
                    <Translate contentKey="fsijhipsterApp.empleado.inmediatosuperior">Inmediatosuperior</Translate>{' '}
                    <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="fsijhipsterApp.empleado.departamento">Departamento</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="fsijhipsterApp.empleado.home.notFound">No Empleados found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
